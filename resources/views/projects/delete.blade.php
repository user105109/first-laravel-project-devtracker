    ...
    <div class="fixed inset-0 flex items-center justify-center z-50 backdrop-blur confirm-dialog ">
        <div class="relative px-4 min-h-screen md:flex md:items-center md:justify-center">
            ...
            <div class="bg-white rounded-lg md:max-w-md md:mx-auto p-4 fixed inset-x-0 bottom-0 z-50 mb-4 mx-4 md:relative shadow-lg">
                ...
                <div class="flex gap-x-2 text-center md:text-right mt-4 md:flex md:justify-end">
                    <form action="/projects/{{ $project->id }}" method="POST">
                        @csrf
                        @method('DELETE') <!-- C'est CA qui dit à Laravel d'aller vers destroy() -->
                        <button id="confirm-delete-btn" class="block w-full md:inline-block md:w-auto px-4 py-3 md:py-2 bg-red-200 text-red-700 rounded-lg font-semibold text-sm md:ml-2 md:order-2" type="submit" >Supprimer</button>
                    </form>
                    <a href="/projects" id="confirm-cancel-btn" class="block w-full md:inline-block md:w-auto px-4 py-3 md:py-2 bg-gray-200 rounded-lg font-semibold text-sm mt-4 md:mt-0 md:order-1">
                        Cancel
                    </a>
                </div>
            </div>
